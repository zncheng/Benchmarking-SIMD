...
double add_avx(const int *a, const int *b){
	...
	for (int i = 0; i < attr_num / 8; i ++){
		...
		_mm256_maskstore_epi32(c + 8 * i, ret, mask_256);
	}
...
double addmul_avx(const int *a, const int *b){
	...
	for (int i = 0; i < attr_num / 8; i ++){
		...
		__m256i z = _mm256_mul_epi32(x, y); 
...
//double addmul_fma(const int *a, const int *b){
//	__attribute__ ((aligned (32))) int c[attr_num] ={0.0};
//	for (int i = 0 ; i < attr_num /8; i ++){
//		__m256 x = _mm256_load_si256(a + 8 * i);
//		__m256 y = _mm256_set_epi32(7+i*8, 6+i*8, 5+i*8, 4+i*8, 3+i*8, 2+i*8, 1+i*8, i*8);
//		__m256 z = _mm256_load_si256(b + 8 *i);
//		__m256 ret = _mm256_fmadd_epi32(x, y, z); //x*y+z
